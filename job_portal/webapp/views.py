...
def login_view(request):
    return render (request,'myhtml/login.html')
...
def fresher_view(request):
    ...
    return render(request,'myhtml/fresher.html',{'form':form})
...
def professional_view(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            print(form.cleaned_data['location'])
            return thankviews(request)
    return render(request,'myhtml/professional.html',{'form':form})

def thankviews(request):
    return render(request, 'myhtml/thanks.html')
